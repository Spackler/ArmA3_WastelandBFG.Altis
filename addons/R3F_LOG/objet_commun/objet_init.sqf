...
if (_fonctionnalites select __can_be_moved_by_player) then
{
	...
	_objet addAction [("<img image='client\icons\r3f_unlock.paa' color='#06ef00'/> <t color='#06ef00'>" + STR_UNLOCK_OBJECT + "</t>"), "addons\R3F_LOG\objet_deplacable\objectLockStateMachine.sqf", _doUnlock, -5, false, true, "", "R3F_LOG_objet_addAction == _target && {R3F_LOG_action_deplacer_objet_valide && {!Object_canLock && {(_target getVariable ['ownerUID','']==getPlayerUID player)}}}"];
};
