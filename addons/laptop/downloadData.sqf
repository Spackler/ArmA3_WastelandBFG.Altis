...
T8_varFileSize = 100172;  								// Filesize ... smaller files will take shorter time to download!
...
T8_varTLine04 = "Download finished! $10,000 added to your account!";				// download finished
	...
	[ _laptop, _id] spawn 
	{
		...
		while { !T8_varDiagAbort && alive player && (player getVariable ["FAR_isUnconscious", 0] == 0)} do
		{
			...
			if ( _newFile > T8_varFileSize ) then 
			{
				...
				axeDiagLog = format ["%1 hacked laptop for %2 money", player, 10000];
